...
def run_train(train_df: pd.DataFrame, strategy: str = "toy"):
    print(f"[TFT] Iniciando TRAIN global probabilistico con variables exogenas...")
    
    models_dir = Path(f"models/{strategy}/tft")
    ...
    try:
        ...
        print(f"[TFT] ERROR de memoria o fitting: {e}")
        # Fallback agresivo
        model_obj = TFT(h=24, input_size=168, max_steps=100, batch_size=8, windows_batch_size=32, accumulate_grad_batches=16,
    ...
    print(f"[TFT] Entrenamiento completado y guardado.")
