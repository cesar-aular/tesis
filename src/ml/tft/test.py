...
def run_test(test_df: pd.DataFrame, silver_dl: pd.DataFrame, results_dir: Path, macrozona: str, estacion: str, planta: str, strategy: str = "toy"):
    print(f"[TFT] Testeando Planta: {planta} ({estacion}) con Roll-Out a 7 dias")
    models_dir = Path(f"models/{strategy}/tft/nf_models")
    
    if not models_dir.exists():
        print(f"[TFT] ERROR: Modelos NeuralForecast no existen.")
        return
    ...
    if len(test_df) < 168 + (7*24):
        print(f"[TFT] Planta {planta} no tiene suficientes datos (168h + 7 dias). Skipiando.")
        return
    ...
    for col in ['TFT-median', 'TFT-lo-90', 'TFT-hi-90']:
    ...
    out_dir_day1 = results_dir / "tft" / macrozona / estacion / planta / "day1"
    out_dir_day1.mkdir(parents=True, exist_ok=True)
    out_day1 = merged_day1.rename(columns={'TFT-median': 'y_pred', 'TFT-lo-90': 'y_pred_lo_90', 'TFT-hi-90': 'y_pred_hi_90'})
    out_day1.to_parquet(out_dir_day1 / "preds.parquet")
    metrics_day1 = calculate_metrics(out_day1['y'], out_day1['y_pred'])
    mark_run_completed(out_dir_day1, "tft_day1", planta, estacion, metrics_day1)
    plot_forecast_rollout(df_real=out_day1, df_pred=out_day1, output_path=out_dir_day1 / "forecast_plot.png", model_name="TFT (1 Day)", planta=planta)
    ...
    for day in range(7):
        ...
        new_hist_tail['y'] = preds['TFT-median'].values # AUTORREGRESIVO (usamos la prediccion, NO LA REALIDAD)
    ...
    out_dir_7d = results_dir / "tft" / macrozona / estacion / planta / "rollout7d"
    out_dir_7d.mkdir(parents=True, exist_ok=True)
    out_7d = merged_7d.rename(columns={'TFT-median': 'y_pred', 'TFT-lo-90': 'y_pred_lo_90', 'TFT-hi-90': 'y_pred_hi_90'})
    out_7d.to_parquet(out_dir_7d / "preds.parquet")
    
    metrics_7d = calculate_metrics(out_7d['y'], out_7d['y_pred'])
    mark_run_completed(out_dir_7d, "tft_rollout7d", planta, estacion, metrics_7d)
    
    # Graficar con bandas de incertidumbre
    plot_forecast_rollout(df_real=out_7d, df_pred=out_7d, output_path=out_dir_7d / "forecast_plot.png", model_name="TFT (7 Day Roll-Out)", planta=planta)

    print(f"[TFT] Roll-Out completado para {planta}.")
