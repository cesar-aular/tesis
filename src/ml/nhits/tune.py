...
def run_tune(train_df: pd.DataFrame, strategy: str = "toy"):
    print(f"[NHITS] Iniciando TUNE global dinamico con variables exogenas...")
    
    out_dir = Path(f"models/{strategy}/nhits")
    ...
    def objective(trial):
        hidden_size = trial.suggest_categorical('hidden_size', [16, 32, 64]) if 'NHITS' == 'TFT' else trial.suggest_categorical('hidden_size', [32, 64, 128])
    ...
    print(f"[NHITS] TUNE completado: {best_params}")
