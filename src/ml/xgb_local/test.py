...
def run_test(test_df: pd.DataFrame, results_dir: Path, macrozona: str, estacion: str, planta: str):
    ...
    if len(test_df) < 168 + (7*24):
        print(f"[xgb_local] Planta {planta} no tiene suficientes datos. Skipiando.")
        return
    ...
    out_dir_day1 = results_dir / "xgb_local" / macrozona / estacion / planta / "day1"
    ...
    mark_run_completed(out_dir_day1, "xgb_local_day1", planta, estacion, metrics_day1)
    plot_forecast_rollout(df_real=out_day1, df_pred=out_day1, output_path=out_dir_day1 / "forecast_plot.png", model_name="xgb_local (1 Day)", planta=planta)
    ...
    out_dir_7d = results_dir / "xgb_local" / macrozona / estacion / planta / "rollout7d"
    ...
    mark_run_completed(out_dir_7d, "xgb_local_rollout7d", planta, estacion, metrics_7d)
    plot_forecast_rollout(df_real=out_7d, df_pred=out_7d, output_path=out_dir_7d / "forecast_plot.png", model_name="xgb_local (7 Day)", planta=planta)
