...
def run_tune(train_df: pd.DataFrame, strategy: str = "toy"):
    print(f"[LSTM] Iniciando TUNE global dinamico con variables exogenas...")
    
    out_dir = Path(f"models/{strategy}/lstm")
    ...
    def objective(trial):
        hidden_size = trial.suggest_categorical('hidden_size', [16, 32, 64]) if 'LSTM' == 'TFT' else trial.suggest_categorical('hidden_size', [32, 64, 128])
    ...
    print(f"[LSTM] TUNE completado: {best_params}")
