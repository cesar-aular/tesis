...
def run_test(test_df: pd.DataFrame, silver_dl: pd.DataFrame, results_dir: Path, macrozona: str, estacion: str, planta: str, strategy: str = "toy"):
    print(f"[LSTM] Testeando Planta: {planta} ({estacion}) con Roll-Out a 7 dias")
    models_dir = Path(f"models/{strategy}/lstm/nf_models")
    
    if not models_dir.exists():
        print(f"[LSTM] ERROR: Modelos NeuralForecast no existen.")
        return
    ...
    if len(test_df) < 168 + (7*24):
        print(f"[LSTM] Planta {planta} no tiene suficientes datos (168h + 7 dias). Skipiando.")
        return
    ...
    for col in ['LSTM-median', 'LSTM-lo-90', 'LSTM-hi-90']:
    ...
    out_dir_day1 = results_dir / "lstm" / macrozona / estacion / planta / "day1"
    out_dir_day1.mkdir(parents=True, exist_ok=True)
    out_day1 = merged_day1.rename(columns={'LSTM-median': 'y_pred', 'LSTM-lo-90': 'y_pred_lo_90', 'LSTM-hi-90': 'y_pred_hi_90'})
    out_day1.to_parquet(out_dir_day1 / "preds.parquet")
    metrics_day1 = calculate_metrics(out_day1['y'], out_day1['y_pred'])
    mark_run_completed(out_dir_day1, "lstm_day1", planta, estacion, metrics_day1)
    plot_forecast_rollout(df_real=out_day1, df_pred=out_day1, output_path=out_dir_day1 / "forecast_plot.png", model_name="LSTM (1 Day)", planta=planta)
    ...
    for day in range(7):
        ...
        new_hist_tail['y'] = preds['LSTM-median'].values # AUTORREGRESIVO (usamos la prediccion, NO LA REALIDAD)
    ...
    out_dir_7d = results_dir / "lstm" / macrozona / estacion / planta / "rollout7d"
    out_dir_7d.mkdir(parents=True, exist_ok=True)
    out_7d = merged_7d.rename(columns={'LSTM-median': 'y_pred', 'LSTM-lo-90': 'y_pred_lo_90', 'LSTM-hi-90': 'y_pred_hi_90'})
    out_7d.to_parquet(out_dir_7d / "preds.parquet")
    
    metrics_7d = calculate_metrics(out_7d['y'], out_7d['y_pred'])
    mark_run_completed(out_dir_7d, "lstm_rollout7d", planta, estacion, metrics_7d)
    
    # Graficar con bandas de incertidumbre
    plot_forecast_rollout(df_real=out_7d, df_pred=out_7d, output_path=out_dir_7d / "forecast_plot.png", model_name="LSTM (7 Day Roll-Out)", planta=planta)

    print(f"[LSTM] Roll-Out completado para {planta}.")
