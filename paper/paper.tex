...
capped per strategy, with early stopping on a 168-hour validation split; hyperparameters are
tuned once per model and cached across LOPO folds (a negligible, documented parameter-level
leakage). Every model emits P5/P50/P95 quantiles (90\,\% intervals), and predictions are
physically constrained ($\hat{y}\!\geq\!0$; forced to zero at night).
...
\textbf{Transferable methodological findings.} Beyond the benchmark, four lessons generalize
...
well---a critical distinction for electricity-market use. (4) Target integrity---never
imputing missing generation---is essential to metric honesty, in contrast to the common
