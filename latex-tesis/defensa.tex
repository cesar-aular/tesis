...
% \minmode=1 -> versión MÍNIMA: deck de la defensa + mapa, sin las diapositivas
% de tests estadísticos ni los respaldos nuevos (ver README_charlas.md).
\providecommand{\minmode}{0}
...
\ifnum\minmode=1
% ---- Versión MÍNIMA: sin mencionar tests estadísticos ----
\begin{frame}{Contraste de la hipótesis}
  ...
  \begin{alertblock}{La hipótesis se \textbf{RECHAZA}}
    Ningún modelo global alcanza al mejor baseline local:
    \lo{XGBoost local} (13,2\,\%) frente a \gl{XGBoost global} (17,1\,\%).
  \end{alertblock}
  \begin{block}{A igualdad de arquitectura tampoco hay ventaja global}
    \begin{itemize}
      \item \lo{N-HiTS local} (17,6\,\%) supera a \gl{N-HiTS global} (30,7\,\%).
      \item El par \textbf{LSTM} queda prácticamente empatado (23,9\,\% vs.\ 24,8\,\%).
    \end{itemize}
  \end{block}
...
\end{frame}
\else
% ---- Versión COMPLETA: con los tests pareados del paper ----
...
% Respaldos exclusivos de la versión COMPLETA (se omiten en \minmode=1)
\ifnum\minmode=0
...
\fi% fin de los respaldos exclusivos de la versión completa
